    ...
    <div className='min-h-screen bg-slate-50'>
      <div className='max-w-6xl mx-auto p-6'>
        ...
        <div className='mb-8'>
          ...
          <p className='text-slate-600'>Manage your network and discover the new  connections</p>
        </div>
        ...
        <div  className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bgowhite shadow-sm'>
          {
            dataArray.map((tab)=>(
              <button
              onClick={()=>setCurrentTab(tab.label)}
               key={tab.label} className={`flex cursor-pointer items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}>
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          ...
          dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
            ...
            >
              <img src={user.profile_picture} alt="" className='w-12 h-12 shadow-md mx-auto' />
                ...
                <div className='flex max-sm:flex-col gap-2 mt-4'>
                  {
                    ...
                    onClick={()=>navigate(`/profile/${user._id}`)}
                     className='w-full p-2 text-sm rounded bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer '>
                      View Profile
