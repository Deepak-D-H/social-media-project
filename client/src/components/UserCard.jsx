...
const UserCard = ({user}) => {
  ...
  const curretnUser = dummyUserData 
   
  const handleFellow = async () =>{
    ...
    key={user._id}
     className='p-4 pt-6 flex flex-col justify-between w-72 shadow border border-gray-200 rounded-md'>
      ...
      <div className='flex items-center justify-center gap-2 mt-4 text-xs text-gray-600'>
        ...
        <div className='flex items-center gap-1 border border-gray-300 rounded-full px-3 py-1'>
          <span>{user.followers.length}</span> Follwers
      ...
      <div className='flex mt-4 gap-2'>
        ...
        disabled = {curretnUser?.following.includes(user._id)}
         className='w-full py-2 rounded-md flex justify-center items-center gap-2 bg-linear-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 active:scale-95 transition text-white cursor-pointer'>
          <UserPlus className = 'w-4 h-4'/>{curretnUser?.following.includes(user._id) ? 'Follwing' : 'Follow'}
        </button>
        ...
        className='flex items-center justify-center w-16 border text-slate-500 group rounded-md cursor-pointer active:scale-95 transition'>
          {
            curretnUser?.connections.includes(user._id) ? <MessageCircle className='w-5 h-5 group-hover:scale-105 transition' />
             :
